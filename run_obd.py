...
def pretty_print_obd_data(obd_data):
    ...
    if "No codes" not in obd_data["DTC_CODES"]:
        print("\n*** VEHICLE TROUBLE CODES DETECTED ***")
        for code, description in obd_data["DTC_CODES"]:
...
def get_data():
    ...
    try:
        ...
        dtcs = obd_man.get_dtc_codes()
        if dtcs:
            filtered_response["DTC_CODES"] = dtcs
        else:
            filtered_response["DTC_CODES"] = "No Codes"
